...
import opensavvy.material3.colors.utils.Color.Companion.fromXyz
import opensavvy.prepared.runner.kotest.PreparedSpec

class ColorTest : PreparedSpec({
	suite("RGB") {
		test("From RGB") {
			check(fromRgb(217, 13, 33) == Color(-2552543))
		}

		test("Red component") {
			check(Color(-2552543).red == 217)
		}

		test("Green component") {
			check(Color(-2552543).green == 13)
		}

		test("Blue component") {
			check(Color(-2552543).blue == 33)
		}
	}

	suite("LinRGB") {
		test("From LinRGB") {
			check(fromLinrgb(doubleArrayOf(0.2, 0.7, 0.6)) == Color(-16313326))
		}
	}

	suite("XYZ") {
		test("From XYZ") {
			check(fromXyz(0.7, 0.9, 0.2) == Color(-15328766))
		}

		test("To XYZ") {
			val (x, y, z) = Color(-15328766).toXyz()
	...
	suite("Lab") {
		test("From Lab") {
			check(fromLab(1.7, 1.9, 1.3) == Color(-15989501))
		}

		test("To Lab") {
			val (l, a, b) = Color(-15989501).toLab()
