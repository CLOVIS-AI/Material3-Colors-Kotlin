 ...
 */
internal fun lerp(start: Double, stop: Double, amount: Double): Double {
 ...
 */
internal fun rotationDirection(from: Double, to: Double): Double {
	val increasingDifference = sanitizeDegreesDouble(to - from)
	return if (increasingDifference <= 180.0) 1.0 else -1.0
}

/** Distance of two points on a circle, represented using degrees.  */
internal fun differenceDegrees(a: Double, b: Double): Double {
	return 180.0 - abs(abs(a - b) - 180.0)
}

/** Multiplies a 1x3 row vector with a 3x3 matrix.  */
internal fun matrixMultiply(row: DoubleArray, matrix: Array<DoubleArray>): DoubleArray {
