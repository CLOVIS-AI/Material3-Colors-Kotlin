...
class MaterialDynamicColors {
	...
	fun surfaceDim(): DynamicColor {
		return DynamicColor(
			name = "surface_dim",
			palette = { it.neutralPalette },
			tone = { if (it.isDark) 6.0 else ContrastCurve(87.0, 87.0, 80.0, 75.0).get(it.contrastLevel) },
			isBackground = true,
			background = null,
			secondBackground = null,
			contrastCurve = null,
	...
	fun surfaceBright(): DynamicColor {
		return DynamicColor(
			name = "surface_bright",
			palette = { it.neutralPalette },
			tone = { if (it.isDark) ContrastCurve(24.0, 24.0, 29.0, 34.0).get(it.contrastLevel) else 98.0 },
			isBackground = true,
			background = null,
			secondBackground = null,
			contrastCurve = null,
	...
	fun surfaceContainerLowest(): DynamicColor {
		return DynamicColor(
			...
			palette = { it.neutralPalette },
			tone = { if (it.isDark) ContrastCurve(4.0, 4.0, 2.0, 0.0).get(it.contrastLevel) else 100.0 },
			isBackground = true,
			background = null,
			secondBackground = null,
			contrastCurve = null,
	...
	fun surfaceContainerLow(): DynamicColor {
		return DynamicColor(
			...
			tone = {
				if (it.isDark)
					ContrastCurve(10.0, 10.0, 11.0, 12.0).get(it.contrastLevel)
				else
					ContrastCurve(96.0, 96.0, 96.0, 95.0).get(it.contrastLevel)
			},
			isBackground = true,
			background = null,
			secondBackground = null,
	...
	fun surfaceContainer(): DynamicColor {
		return DynamicColor(
			...
			tone = {
				if (it.isDark)
					ContrastCurve(12.0, 12.0, 16.0, 20.0).get(it.contrastLevel)
				else
					ContrastCurve(94.0, 94.0, 92.0, 90.0).get(it.contrastLevel)
			},
			isBackground = true,
			background = null,
			secondBackground = null,
	...
	fun surfaceContainerHigh(): DynamicColor {
		return DynamicColor(
			...
			tone = {
				if (it.isDark)
					ContrastCurve(17.0, 17.0, 21.0, 25.0).get(it.contrastLevel)
				else
					ContrastCurve(92.0, 92.0, 88.0, 85.0).get(it.contrastLevel)
			},
			isBackground = true,
			background = null,
			secondBackground = null,
	...
	fun surfaceContainerHighest(): DynamicColor {
		return DynamicColor(
			...
			tone = {
				if (it.isDark)
					ContrastCurve(22.0, 22.0, 26.0, 30.0).get(it.contrastLevel)
				else
					ContrastCurve(90.0, 90.0, 84.0, 80.0).get(it.contrastLevel)
			},
			isBackground = true,
			background = null,
			secondBackground = null,
	...
	fun onSurface(): DynamicColor {
		return DynamicColor(
			...
			tone = { if (it.isDark) 90.0 else 10.0 },
			isBackground = false,
			background = { it: DynamicScheme -> this.highestSurface(it) },
			secondBackground = null,
	...
	fun onPrimaryContainer(): DynamicColor {
		return DynamicColor(
			...
			tone = { s ->
				if (isFidelity(s)) {
					return@DynamicColor DynamicColor.foregroundTone(primaryContainer().tone.invoke(s), 4.5)
				}
				if (isMonochrome(s)) {
	...
	fun onSecondaryContainer(): DynamicColor {
		return DynamicColor(
			...
			tone = { s ->
				...
				if (!isFidelity(s)) {
					return@DynamicColor if (s.isDark) 90.0 else 30.0
				}
